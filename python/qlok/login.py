...
class On_Closing():
    def __init__(self,name):
      self.name=name
    def on_closing(self):
        if msg.askokcancel(f"Confirm Message", "你确定要退出当前窗口吗？"):
            self.name.destroy()
            self.name.quit()
    def on_close(self):
        if msg.askokcancel(f"Confirm Message", "你确定要退出当前窗口吗？"):
...
#         print('用户名不存在！')
clo1=On_Closing(win)
win.protocol("WM_DELETE_WINDOW", clo1.on_closing)
